...
func (q *Queries) SearchSpotsByEmbedding(ctx context.Context, embedding *pgvector.Vector) ([]Spot, error) {
	rows, err := q.db.Query(ctx, searchSpotsByEmbedding, embedding)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
