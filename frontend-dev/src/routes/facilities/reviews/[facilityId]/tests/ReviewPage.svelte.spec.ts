...
describe('Review Submission Page', () => {
  ...
  it('マウント時に施設詳細と既存レビューが表示される', async () => {
    ...
    // 詳細表示ボタンをクリックします
    const detailsButton = screen.getByRole('button', { name: /東京タワー/ });
  ...
  it('フォームを入力して確認し、レビューを投稿すると画面遷移が起こる', async () => {
    ...
    // ユーザー入力をシミュレートします
    await fireEvent.input(contentTextarea, { target: { value: '最高の体験でした！' } });
    ...
    // 確認画面が表示されたことを確認します
    expect(screen.getByText('以下の内容で投稿します。よろしいですか？')).toBeInTheDocument();
